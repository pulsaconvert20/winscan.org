...
The domain whitelist feature allows you to restrict which domains can access your WinScan Explorer instance. This is useful when you want to prevent unauthorized domains from using your backend SSL infrastructure.

### How It Works

When a request comes to your explorer, the middleware checks if the requesting domain is in the whitelist. If not, it returns a 403 Forbidden error.
...
|---------------|-------------|
| **Empty value** | Allows all domains (no restrictions) |
| **Single domain** | `NEXT_PUBLIC_ALLOWED_DOMAINS=winscan.org` |
| **Multiple domains** | `NEXT_PUBLIC_ALLOWED_DOMAINS=winscan.org,winscan.winsnip.xyz,localhost` |

### Examples

#### Example 1: Allow All Domains (Default)
```env
NEXT_PUBLIC_ALLOWED_DOMAINS=
```
- No restrictions
- Any domain can access the explorer
- Good for public explorers

#### Example 2: Restrict to Specific Domains
```env
NEXT_PUBLIC_ALLOWED_DOMAINS=winscan.org,winscan.winsnip.xyz
```
- Only `winscan.org` and `winscan.winsnip.xyz` can access
- Other domains get 403 Forbidden error
- Good for private/controlled deployments

#### Example 3: Include Localhost for Development
```env
NEXT_PUBLIC_ALLOWED_DOMAINS=winscan.org,winscan.winsnip.xyz,localhost
```
- Production domains + localhost
...
1. **Exact domain match:**
   - `winscan.org` matches `winscan.org`
   - `localhost` matches `localhost`

2. **Subdomain match:**
   - `winscan.org` also matches `*.winscan.org`
   - Examples: `api.winscan.org`, `test.winscan.org`

3. **Port handling:**
   - Ports are automatically stripped
   - `localhost:3000` matches `localhost`

### Error Response

When a domain is not whitelisted, users see:
...
#### Use Case 1: Backend SSL Protection
Prevent unauthorized domains from using your backend SSL infrastructure:

```env
# Only allow registered domains in backend SSL
NEXT_PUBLIC_ALLOWED_DOMAINS=winscan.org,winscan.winsnip.xyz
```

#### Use Case 2: Multi-Environment Setup
Different whitelists for different environments:
...
**Staging (.env.staging):**
```env
NEXT_PUBLIC_ALLOWED_DOMAINS=staging.winscan.org,localhost
```
...
Start with restricted access, then open up:

**Phase 1 - Beta:**
```env
NEXT_PUBLIC_ALLOWED_DOMAINS=beta.winscan.org
```
...
3. **Test access:**
   ...
   - ❌ `http://127.0.0.1:3000` - Should be blocked (if not in whitelist)

#### Test in Production

1. **Deploy with whitelist:**
   ```env
   NEXT_PUBLIC_ALLOWED_DOMAINS=winscan.org
   ```

2. **Test authorized domain:**
   ```bash
   curl https://winscan.org
   # Should return normal response
   ```

3. **Test unauthorized domain:**
   ```bash
...
1. **Always include localhost during development:**
   ```env
   NEXT_PUBLIC_ALLOWED_DOMAINS=winscan.org,localhost
   ```

2. **Use specific domains, not wildcards:**
   - ✅ Good: `winscan.org,api.winscan.org`
   - ❌ Bad: `*.org` (not supported)

3. **Keep whitelist minimal:**
   - Only add domains you control
   - Remove unused domains

4. **Document your whitelist:**
   - Keep track of why each domain is whitelisted
   - Review periodically

5. **Test before deploying:**
   - Always test whitelist changes locally
   - Verify authorized domains work
   - Verify unauthorized domains are blocked

### Troubleshooting

#### Problem: Legitimate domain blocked

**Solution:**
1. Check domain spelling in whitelist
2. Ensure no extra spaces: `domain1,domain2` not `domain1, domain2`
3. Check if domain includes port (ports are stripped automatically)
...
3. Check middleware.ts is not modified
4. Verify environment variable is loaded: `console.log(process.env.NEXT_PUBLIC_ALLOWED_DOMAINS)`

#### Problem: Localhost blocked
