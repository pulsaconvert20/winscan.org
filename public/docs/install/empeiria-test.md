...
```bash
curl -Ls https://data.winsnip.xyz/list/testnet/empeiria/genesis.json > $HOME/.empeiria-test/config/genesis.json
```

# Download addrbook

```bash
curl -Ls https://data.winsnip.xyz/list/testnet/empeiria/addrbook.json > $HOME/.empeiria-test/config/addrbook.json
```
