...
rm -rf bin
mkdir bin && cd bin
wget -O wardend https://warden-public-bucket.s3.eu-west-1.amazonaws.com/wardend-v1.0.0-rc1-1-gabb089391-linux-amd64
chmod +x wardend
...
```bash
curl -Ls https://server-2.itrocket.net/mainnet/warden/genesis.json > $HOME/.warden-mainnet/config/genesis.json
...
```bash
curl -Ls https://server-2.itrocket.net/mainnet/warden/addrbook.json > $HOME/.warden-mainnet/config/addrbook.json
```
