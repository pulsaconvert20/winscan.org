...
```bash
curl -Ls https://server-4.itrocket.net/mainnet/lava/genesis.json > $HOME/.lava-mainnet/config/genesis.json
```

# Download addrbook

```bash
curl -Ls https://server-4.itrocket.net/mainnet/lava/addrbook.json > $HOME/.lava-mainnet/config/addrbook.json
```
