...
```bash
curl -Ls https://server-4.itrocket.net/mainnet/xrplevm/genesis.json > $HOME/.xrpl-mainnet/config/genesis.json
```

# Download addrbook

```bash
curl -Ls https://server-4.itrocket.net/mainnet/xrplevm/addrbook.json > $HOME/.xrpl-mainnet/config/addrbook.json
```
