# Osmosis Testnet Node Installation Guide

## Chain Information

- **Chain ID**: `osmo-test-5`
- **Network Type**: Testnet
- **Recommended Version**: `v28.0.0`
- **Bech32 Prefix**: `osmo`
...
```bash
curl -Ls https://lcd.osmotest5.osmosis.zone/genesis > $HOME/.osmosis-test/config/genesis.json
...
```bash
curl -Ls https://lcd.osmotest5.osmosis.zone/addrbook > $HOME/.osmosis-test/config/addrbook.json
```
...
sed -i 's/minimum-gas-prices = ""/minimum-gas-prices = "0.0025uosmo"/' $HOME/.osmosis-test/config/app.toml

# Set pruning (optional)
sed -i 's/pruning = "default"/pruning = "custom"/' $HOME/.osmosis-test/config/app.toml
sed -i 's/pruning-keep-recent = "0"/pruning-keep-recent = "100"/' $HOME/.osmosis-test/config/app.toml
sed -i 's/pruning-interval = "0"/pruning-interval = "10"/' $HOME/.osmosis-test/config/app.toml

# Set peers
PEERS="PEER_LIST_HERE"
sed -i "s/persistent_peers = \"\"/persistent_peers = \"$PEERS\"/" $HOME/.osmosis-test/config/config.toml

# Set seeds (if available)
SEEDS="SEED_LIST_HERE"
sed -i "s/seeds = \"\"/seeds = \"$SEEDS\"/" $HOME/.osmosis-test/config/config.toml
```

### 5. Create Systemd Service

```bash
sudo tee /etc/systemd/system/osmosis-testd.service > /dev/null <<EOF
[Unit]
...
sudo systemctl enable osmosis-testd
```
...
```bash
# Start service
sudo systemctl start osmosis-testd

# Check logs
sudo journalctl -u osmosis-testd -f -o cat
...
```bash
# Stop node
sudo systemctl stop osmosis-testd

# Reset data
$(basename https://github.com/osmosis-labs/osmosis .git || chainName)d tendermint unsafe-reset-all --home $HOME/.osmosis-test

# Get trust height and hash
SNAP_RPC="https://rpc.osmotest5.osmosis.zone"
...
s|^(trust_hash[[:space:]]+=[[:space:]]+).*$|\1\"$TRUST_HASH\"|" $HOME/.osmosis-test/config/config.toml
...
### Restart Node
```bash
sudo systemctl restart osmosis-testd
```
