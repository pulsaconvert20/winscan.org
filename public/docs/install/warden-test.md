...
chmod +x wardend
sudo mv $HOME/bin/wardend $HOME/go/bin
```
...
```bash
curl -Ls https://server-3.itrocket.net/testnet/warden/genesis.json > $HOME/.warden-test/config/genesis.json
```

# Download addrbook

```bash
curl -Ls https://server-3.itrocket.net/testnet/warden/addrbook.json > $HOME/.warden-test/config/addrbook.json
```
...
sed -i 's/minimum-gas-prices = ""/minimum-gas-prices = "250000000award"/' $HOME/.warden-test/config/app.toml

# Set pruning (optional)
sed -i 's/pruning = "default"/pruning = "custom"/' $HOME/.warden-test/config/app.toml
sed -i 's/pruning-keep-recent = "0"/pruning-keep-recent = "100"/' $HOME/.warden-test/config/app.toml
sed -i 's/pruning-interval = "0"/pruning-interval = "10"/' $HOME/.warden-test/config/app.toml

# Set peers
PEERS="PEER_LIST_HERE"
sed -i "s/persistent_peers = \"\"/persistent_peers = \"$PEERS\"/" $HOME/.warden-test/config/config.toml

# Set seeds (if available)
SEEDS="SEED_LIST_HERE"
sed -i "s/seeds = \"\"/seeds = \"$SEEDS\"/" $HOME/.warden-test/config/config.toml
```

### 5. Create Systemd Service

```bash
sudo tee /etc/systemd/system/warden-testd.service > /dev/null <<EOF
[Unit]
Description=Warden Protocol Testnet Node
...
sudo systemctl enable warden-testd
```
...
```bash
# Start service
sudo systemctl start warden-testd

# Check logs
sudo journalctl -u warden-testd -f -o cat
...
```bash
# Stop node
sudo systemctl stop warden-testd

# Reset data
$(basename https://github.com/warden-protocol/wardenprotocol .git || chainName)d tendermint unsafe-reset-all --home $HOME/.warden-test

# Get trust height and hash
SNAP_RPC="https://warden-testnet-rpc.polkachu.com"
...
s|^(trust_hash[[:space:]]+=[[:space:]]+).*$|\1\"$TRUST_HASH\"|" $HOME/.warden-test/config/config.toml
...
### Restart Node
```bash
sudo systemctl restart warden-testd
```
...
- **RPC**: https://warden-testnet-rpc.polkachu.com
- **API**: https://warden-testnet-api.polkachu.com
