...
```bash
curl -Ls https://snapshots.polkachu.com/genesis/osmosis/genesis.json > $HOME/.osmosis-mainnet/config/genesis.json
```

# Download addrbook

```bash
curl -Ls https://osmosis-api.polkachu.com/addrbook > $HOME/.osmosis-mainnet/config/addrbook.json
```
