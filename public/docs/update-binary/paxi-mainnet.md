# Paxi Network Binary Update Guide

This guide will help you update your Paxi Network node to the latest version.

## Prerequisites

- Existing Paxi Network node installation
- Root or sudo access
...
cp $HOME/.paxi/config/priv_validator_key.json $HOME/priv_validator_key.json.backup

# Backup node key
cp $HOME/.paxi/config/node_key.json $HOME/node_key.json.backup
```
...
```bash
cd $HOME
wget -O paxid.tar.gz "$(curl -s https://api.github.com/repos/paxi-web3/paxi/releases/latest | jq -r '.assets[] | select(.name | contains("linux") and (contains("amd64") or contains("x86_64"))) | .browser_download_url' | head -1)"
tar -xzf paxid.tar.gz
rm -f install.sh paxid.tar.gz
chmod +x paxid
sudo mv paxid /usr/local/bin/
...
```bash
sudo cp /usr/local/bin/paxid.backup /usr/local/bin/paxid
```
...
```bash
mkdir -p $HOME/.paxi/cosmovisor/upgrades/upgrade-1.0.0/bin
```
...
tar -xzf paxid.tar.gz
chmod +x paxid
mv paxid $HOME/.paxi/cosmovisor/upgrades/upgrade-1.0.0/bin/
```

### 3. Verify Binary

```bash
$HOME/.paxi/cosmovisor/upgrades/upgrade-1.0.0/bin/paxid version
```
...
```bash
sudo journalctl -u paxid -n 100 --no-pager
```

Common fixes:
- Reset the node data (if not a validator): `paxid tendermint unsafe-reset-all --home $HOME/.paxi`
...
# Reset data
paxid tendermint unsafe-reset-all --home $HOME/.paxi --keep-addr-book
...
- [Official Paxi Network Documentation](https://github.com/paxi-web3/paxi)
