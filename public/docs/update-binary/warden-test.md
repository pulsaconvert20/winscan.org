# Warden Protocol Binary Update Guide

This guide will help you update your Warden Protocol node to the latest version.

## Prerequisites

- Existing Warden Protocol node installation
- Root or sudo access
...
cp $HOME/.warden/config/priv_validator_key.json $HOME/priv_validator_key.json.backup

# Backup node key
cp $HOME/.warden/config/node_key.json $HOME/node_key.json.backup
```
...
```bash
cd $HOME
wget -O wardend.tar.gz "$(curl -s https://api.github.com/repos/warden-protocol/wardenprotocol/releases/latest | jq -r '.assets[] | select(.name | contains("linux") and (contains("amd64") or contains("x86_64"))) | .browser_download_url' | head -1)"
tar -xzf wardend.tar.gz
rm -f install.sh wardend.tar.gz
chmod +x wardend
...
```bash
sudo cp /usr/local/bin/wardend.backup /usr/local/bin/wardend
```
...
```bash
mkdir -p $HOME/.warden/cosmovisor/upgrades/upgrade-latest/bin
```
...
tar -xzf wardend.tar.gz
chmod +x wardend
mv wardend $HOME/.warden/cosmovisor/upgrades/upgrade-latest/bin/
```

### 3. Verify Binary

```bash
$HOME/.warden/cosmovisor/upgrades/upgrade-latest/bin/wardend version
```
...
```bash
sudo journalctl -u wardend -n 100 --no-pager
```

Common fixes:
- Reset the node data (if not a validator): `wardend tendermint unsafe-reset-all --home $HOME/.warden`
...
- [Official Warden Protocol Documentation](https://github.com/warden-protocol/wardenprotocol)
