# Cysic Network Binary Update Guide

This guide will help you update your Cysic Network node to the latest version.

## Prerequisites

- Existing Cysic Network node installation
- Root or sudo access
...
cp $HOME/.cysic/config/priv_validator_key.json $HOME/priv_validator_key.json.backup

# Backup node key
cp $HOME/.cysic/config/node_key.json $HOME/node_key.json.backup
```
...
```bash
cd $HOME
wget -O cysicd.tar.gz "$(curl -s https://api.github.com/repos//releases/latest | jq -r '.assets[] | select(.name | contains("linux") and (contains("amd64") or contains("x86_64"))) | .browser_download_url' | head -1)"
tar -xzf cysicd.tar.gz
rm -f install.sh cysicd.tar.gz
chmod +x cysicd
sudo mv cysicd /usr/local/bin/
...
```bash
sudo cp /usr/local/bin/cysicd.backup /usr/local/bin/cysicd
```

Or reinstall the previous version:

```bash
cd $HOME/cysic-mainnet
git checkout v1.0.0
make install
```
...
```bash
mkdir -p $HOME/.cysic/cosmovisor/upgrades/upgrade-latest/bin
```
...
tar -xzf cysicd.tar.gz
chmod +x cysicd
mv cysicd $HOME/.cysic/cosmovisor/upgrades/upgrade-latest/bin/
```

### 3. Verify Binary

```bash
$HOME/.cysic/cosmovisor/upgrades/upgrade-latest/bin/cysicd version
```
...
```bash
sudo journalctl -u cysicd -n 100 --no-pager
```

Common fixes:
- Reset the node data (if not a validator): `cysicd tendermint unsafe-reset-all --home $HOME/.cysic`
...
# Reset data
cysicd tendermint unsafe-reset-all --home $HOME/.cysic --keep-addr-book
...
- [Official Cysic Network Documentation]()
