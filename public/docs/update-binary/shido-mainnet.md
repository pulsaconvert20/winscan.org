# Shido Network Binary Update Guide

This guide will help you update your Shido Network node to the latest version.

## Prerequisites

- Existing Shido Network node installation
- Root or sudo access
...
cp $HOME/.shido/config/priv_validator_key.json $HOME/priv_validator_key.json.backup

# Backup node key
cp $HOME/.shido/config/node_key.json $HOME/node_key.json.backup
```
...
```bash
cd $HOME
wget -O shidod.tar.gz "$(curl -s https://api.github.com/repos/ShidoGlobal/shido-upgrade-v3.3.1/releases/latest | jq -r '.assets[] | select(.name | contains("linux") and (contains("amd64") or contains("x86_64"))) | .browser_download_url' | head -1)"
tar -xzf shidod.tar.gz
rm -f install.sh shidod.tar.gz
chmod +x shidod
...
```bash
sudo cp /usr/local/bin/shidod.backup /usr/local/bin/shidod
```

Or reinstall the previous version:

```bash
cd $HOME/shido-upgrade-v3.3.1
git checkout v1.0.0
make install
```
...
```bash
mkdir -p $HOME/.shido/cosmovisor/upgrades/upgrade-latest/bin
```
...
tar -xzf shidod.tar.gz
chmod +x shidod
mv shidod $HOME/.shido/cosmovisor/upgrades/upgrade-latest/bin/
```

### 3. Verify Binary

```bash
$HOME/.shido/cosmovisor/upgrades/upgrade-latest/bin/shidod version
```
...
```bash
sudo journalctl -u shidod -n 100 --no-pager
```

Common fixes:
- Reset the node data (if not a validator): `shidod tendermint unsafe-reset-all --home $HOME/.shido`
...
# Reset data
shidod tendermint unsafe-reset-all --home $HOME/.shido --keep-addr-book
...
- [Official Shido Network Documentation](https://github.com/ShidoGlobal/shido-upgrade-v3.3.1)
