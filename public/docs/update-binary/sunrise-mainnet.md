# Sunrise Binary Update Guide

This guide will help you update your Sunrise node to the latest version.

## Prerequisites

- Existing Sunrise node installation
- Root or sudo access
...
cp $HOME/.sunrise/config/priv_validator_key.json $HOME/priv_validator_key.json.backup

# Backup node key
cp $HOME/.sunrise/config/node_key.json $HOME/node_key.json.backup
```
...
```bash
cd $HOME
wget -O sunrised.tar.gz "$(curl -s https://api.github.com/repos/sunriselayer/sunrise/releases/latest | jq -r '.assets[] | select(.name | contains("linux") and (contains("amd64") or contains("x86_64"))) | .browser_download_url' | head -1)"
tar -xzf sunrised.tar.gz
rm -f install.sh sunrised.tar.gz
chmod +x sunrised
...
```bash
sudo cp /usr/local/bin/sunrised.backup /usr/local/bin/sunrised
```
...
```bash
mkdir -p $HOME/.sunrise/cosmovisor/upgrades/upgrade-1.0.0/bin
```
...
tar -xzf sunrised.tar.gz
chmod +x sunrised
mv sunrised $HOME/.sunrise/cosmovisor/upgrades/upgrade-1.0.0/bin/
```

### 3. Verify Binary

```bash
$HOME/.sunrise/cosmovisor/upgrades/upgrade-1.0.0/bin/sunrised version
```
...
```bash
sudo journalctl -u sunrised -n 100 --no-pager
```

Common fixes:
- Reset the node data (if not a validator): `sunrised tendermint unsafe-reset-all --home $HOME/.sunrise`
...
- [Official Sunrise Documentation](https://github.com/sunriselayer/sunrise)
