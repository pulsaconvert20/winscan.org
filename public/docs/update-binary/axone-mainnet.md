# Axone Binary Update Guide

This guide will help you update your Axone node to the latest version.

## Prerequisites

- Existing Axone node installation
- Root or sudo access
...
cp $HOME/.axone/config/priv_validator_key.json $HOME/priv_validator_key.json.backup

# Backup node key
cp $HOME/.axone/config/node_key.json $HOME/node_key.json.backup
```
...
```bash
cd $HOME
wget -O axoned.tar.gz "$(curl -s https://api.github.com/repos/axone-protocol/axoned/releases/latest | jq -r '.assets[] | select(.name | contains("linux") and (contains("amd64") or contains("x86_64"))) | .browser_download_url' | head -1)"
tar -xzf axoned.tar.gz
rm -f install.sh axoned.tar.gz
chmod +x axoned
sudo mv axoned /usr/local/bin/
...
```bash
sudo cp /usr/local/bin/axoned.backup /usr/local/bin/axoned
```
...
```bash
mkdir -p $HOME/.axone/cosmovisor/upgrades/upgrade-9.0.0/bin
```
...
tar -xzf axoned.tar.gz
chmod +x axoned
mv axoned $HOME/.axone/cosmovisor/upgrades/upgrade-9.0.0/bin/
```

### 3. Verify Binary

```bash
$HOME/.axone/cosmovisor/upgrades/upgrade-9.0.0/bin/axoned version
```
...
```bash
sudo journalctl -u axoned -n 100 --no-pager
```

Common fixes:
- Reset the node data (if not a validator): `axoned tendermint unsafe-reset-all --home $HOME/.axone`
...
# Reset data
axoned tendermint unsafe-reset-all --home $HOME/.axone --keep-addr-book
...
- [Official Axone Documentation](https://github.com/axone-protocol/axoned)
