# Noble Binary Update Guide

This guide will help you update your Noble node to the latest version.

## Prerequisites

- Existing Noble node installation
- Root or sudo access
...
cp $HOME/.noble/config/priv_validator_key.json $HOME/priv_validator_key.json.backup

# Backup node key
cp $HOME/.noble/config/node_key.json $HOME/node_key.json.backup
```
...
```bash
cd $HOME
wget -O nobled.tar.gz "$(curl -s https://api.github.com/repos/noble-assets/noble/releases/latest | jq -r '.assets[] | select(.name | contains("linux") and (contains("amd64") or contains("x86_64"))) | .browser_download_url' | head -1)"
tar -xzf nobled.tar.gz
rm -f install.sh nobled.tar.gz
chmod +x nobled
sudo mv nobled /usr/local/bin/
...
```bash
sudo cp /usr/local/bin/nobled.backup /usr/local/bin/nobled
```
...
```bash
mkdir -p $HOME/.noble/cosmovisor/upgrades/upgrade-4.0.0/bin
```
...
tar -xzf nobled.tar.gz
chmod +x nobled
mv nobled $HOME/.noble/cosmovisor/upgrades/upgrade-4.0.0/bin/
```

### 3. Verify Binary

```bash
$HOME/.noble/cosmovisor/upgrades/upgrade-4.0.0/bin/nobled version
```
...
```bash
sudo journalctl -u nobled -n 100 --no-pager
```

Common fixes:
- Reset the node data (if not a validator): `nobled tendermint unsafe-reset-all --home $HOME/.noble`
...
- [Official Noble Documentation](https://github.com/noble-assets/noble)
