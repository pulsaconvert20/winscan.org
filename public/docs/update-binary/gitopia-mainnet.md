# Gitopia Binary Update Guide

This guide will help you update your Gitopia node to the latest version.

## Prerequisites

- Existing Gitopia node installation
- Root or sudo access
...
cp $HOME/.gitopia/config/priv_validator_key.json $HOME/priv_validator_key.json.backup

# Backup node key
cp $HOME/.gitopia/config/node_key.json $HOME/node_key.json.backup
```
...
```bash
cd $HOME
wget -O gitopiad.tar.gz "$(curl -s https://api.github.com/repos/gitopia/gitopia/releases/latest | jq -r '.assets[] | select(.name | contains("linux") and (contains("amd64") or contains("x86_64"))) | .browser_download_url' | head -1)"
tar -xzf gitopiad.tar.gz
rm -f install.sh gitopiad.tar.gz
chmod +x gitopiad
sudo mv gitopiad /usr/local/bin/
...
```bash
sudo cp /usr/local/bin/gitopiad.backup /usr/local/bin/gitopiad
```
...
```bash
mkdir -p $HOME/.gitopia/cosmovisor/upgrades/upgrade-4.0.0/bin
```
...
tar -xzf gitopiad.tar.gz
chmod +x gitopiad
mv gitopiad $HOME/.gitopia/cosmovisor/upgrades/upgrade-4.0.0/bin/
```

### 3. Verify Binary

```bash
$HOME/.gitopia/cosmovisor/upgrades/upgrade-4.0.0/bin/gitopiad version
```
...
```bash
sudo journalctl -u gitopiad -n 100 --no-pager
```

Common fixes:
- Reset the node data (if not a validator): `gitopiad tendermint unsafe-reset-all --home $HOME/.gitopia`
...
- [Official Gitopia Documentation](https://github.com/gitopia/gitopia)
