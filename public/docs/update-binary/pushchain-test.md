# Push Chain Binary Update Guide

This guide will help you update your Push Chain node to the latest version.

## Prerequisites

- Existing Push Chain node installation
- Root or sudo access
...
cp $HOME/.pushchain/config/priv_validator_key.json $HOME/priv_validator_key.json.backup

# Backup node key
cp $HOME/.pushchain/config/node_key.json $HOME/node_key.json.backup
```
...
```bash
cd $HOME
wget -O pushchaind.tar.gz "$(curl -s https://api.github.com/repos/pushchain/push-chain-node/releases/latest | jq -r '.assets[] | select(.name | contains("linux") and (contains("amd64") or contains("x86_64"))) | .browser_download_url' | head -1)"
tar -xzf pushchaind.tar.gz
rm -f install.sh pushchaind.tar.gz
chmod +x pushchaind
sudo mv pushchaind /usr/local/bin/
...
```bash
sudo cp /usr/local/bin/pushchaind.backup /usr/local/bin/pushchaind
```

Or reinstall the previous version:

```bash
cd $HOME/push-chain-node
git checkout v1.0.0
make install
```
...
```bash
mkdir -p $HOME/.pushchain/cosmovisor/upgrades/upgrade-0.0.15/bin
```
...
tar -xzf pushchaind.tar.gz
chmod +x pushchaind
mv pushchaind $HOME/.pushchain/cosmovisor/upgrades/upgrade-0.0.15/bin/
```

### 3. Verify Binary

```bash
$HOME/.pushchain/cosmovisor/upgrades/upgrade-0.0.15/bin/pushchaind version
```
...
```bash
sudo journalctl -u pushchaind -n 100 --no-pager
```

Common fixes:
- Reset the node data (if not a validator): `pushchaind tendermint unsafe-reset-all --home $HOME/.pushchain`
...
# Reset data
pushchaind tendermint unsafe-reset-all --home $HOME/.pushchain --keep-addr-book
...
- [Official Push Chain Documentation](https://github.com/pushchain/push-chain-node)
