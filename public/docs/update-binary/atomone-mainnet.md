...
```bash
mkdir -p $HOME/.atomone/cosmovisor/upgrades/upgrade-3.0.3/bin
```
...
tar -xzf atomoned.tar.gz
chmod +x atomoned
mv atomoned $HOME/.atomone/cosmovisor/upgrades/upgrade-3.0.3/bin/
```

### 3. Verify Binary

```bash
$HOME/.atomone/cosmovisor/upgrades/upgrade-3.0.3/bin/atomoned version
```
