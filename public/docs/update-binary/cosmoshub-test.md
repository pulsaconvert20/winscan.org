# Cosmos Hub Binary Update Guide

This guide will help you update your Cosmos Hub node to the latest version.

## Prerequisites

- Existing Cosmos Hub node installation
- Root or sudo access
...
cp $HOME/.cosmoshub/config/priv_validator_key.json $HOME/priv_validator_key.json.backup

# Backup node key
cp $HOME/.cosmoshub/config/node_key.json $HOME/node_key.json.backup
```
...
```bash
cd $HOME
wget -O cosmoshubd.tar.gz "$(curl -s https://api.github.com/repos/cosmos/gaia/releases/latest | jq -r '.assets[] | select(.name | contains("linux") and (contains("amd64") or contains("x86_64"))) | .browser_download_url' | head -1)"
tar -xzf cosmoshubd.tar.gz
rm -f install.sh cosmoshubd.tar.gz
chmod +x cosmoshubd
sudo mv cosmoshubd /usr/local/bin/
...
```bash
sudo cp /usr/local/bin/cosmoshubd.backup /usr/local/bin/cosmoshubd
```
...
```bash
mkdir -p $HOME/.cosmoshub/cosmovisor/upgrades/upgrade-20.0.0/bin
```
...
tar -xzf cosmoshubd.tar.gz
chmod +x cosmoshubd
mv cosmoshubd $HOME/.cosmoshub/cosmovisor/upgrades/upgrade-20.0.0/bin/
```

### 3. Verify Binary

```bash
$HOME/.cosmoshub/cosmovisor/upgrades/upgrade-20.0.0/bin/cosmoshubd version
```
...
```bash
sudo journalctl -u cosmoshubd -n 100 --no-pager
```

Common fixes:
- Reset the node data (if not a validator): `cosmoshubd tendermint unsafe-reset-all --home $HOME/.cosmoshub`
...
# Reset data
cosmoshubd tendermint unsafe-reset-all --home $HOME/.cosmoshub --keep-addr-book
...
- [Official Cosmos Hub Documentation](https://github.com/cosmos/gaia)
