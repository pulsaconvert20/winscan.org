...
export default function EVMPendingPage() {
  ...
  // Fetch pending tx data from real API
  useEffect(() => {
    if (!selectedChain) return;
    setLoading(true);
    const chainName = selectedChain.chain_name.toLowerCase().replace(/\s+/g, '-');
    fetch(`/api/evm/pending?chain=${chainName}`)
      .then(res => res.json())
      .then(data => {
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto bg-[#0a0a0a]">
          <div className="container mx-auto px-3 md:px-6 py-6 md:py-8 pt-24 md:pt-8">
            <div className="mb-4 md:mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div>
                <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">EVM Pending Transactions</h1>
                <p className="text-gray-400 text-sm md:text-base">Pending transactions for {selectedChain?.chain_name || ''}</p>
              </div>
              {!isRefreshing && (
                  ...
                  <table className="min-w-full divide-y divide-gray-800">
                    <thead className="bg-[#0f0f0f]">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Gas Price</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Nonce</th>
                      </tr>
                    </thead>
                    <tbody className="bg-[#1a1a1a] divide-y divide-gray-800">
                      {pending.map((tx, i) => (
                        <tr key={i}>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-200">{tx.hash}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-200">{tx.from}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-200">{tx.to || '-'}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-200">{tx.value}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-200">{tx.gasPrice}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-200">{tx.nonce !== undefined ? tx.nonce : '-'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
